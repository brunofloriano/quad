...
void command::write_cw_angle_limit(dynamixel::PortHandler *portHandler, dynamixel::PacketHandler *packetHandler, uint8_t id, uint32_t value)
{
    uint8_t dxl_error = 0;
    int dxl_comm_result = COMM_TX_FAIL;

    do{
        dxl_comm_result = packetHandler->write2ByteTxRx(portHandler, id, 6, (uint16_t)value, &dxl_error);
        if (dxl_comm_result == COMM_SUCCESS)
        {
            if (dxl_error != 0) packetHandler->printRxPacketError(dxl_error);
            //fprintf(stderr, "\n Success to write\n\n");
...
void command::write_ccw_angle_limit(dynamixel::PortHandler *portHandler, dynamixel::PacketHandler *packetHandler, uint8_t id, uint32_t value)
{
    uint8_t dxl_error = 0;
    int dxl_comm_result = COMM_TX_FAIL;

    do{
        dxl_comm_result = packetHandler->write2ByteTxRx(portHandler, id, 8, (uint16_t)value, &dxl_error);
        if (dxl_comm_result == COMM_SUCCESS)
        {
            if (dxl_error != 0) packetHandler->printRxPacketError(dxl_error);
            //fprintf(stderr, "\n Success to write\n\n");
